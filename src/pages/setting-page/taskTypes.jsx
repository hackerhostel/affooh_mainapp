    ...
    if (confirmDeleteId) {
      try {
        ...
        if (status === 200 || status === 204) {
          toast.success("Task type deleted successfully!", { appearance: "success" });
          dispatch(fetchAllTaskTypes());
          setCurrentPage(1);
            dispatch(doGetMasterData())
        } else {
          toast.success("Failed to delete Task Type", { appearance: "error" });
        }
      } catch (error) {
        toast.success("Failed to delete Task Type", { appearance: "error" });
      } finally {
    ...
    <div className="p-3 bg-dashboard-bgc h-full">
      <div>
        <div className="flex items-center justify-between p-4">
          ...
          <div className={"flex gap-1 items-center mr-5"} onClick={() => setNewCustomField(true)}><PlusCircleIcon className={"w-6 h-6 text-pink-500 cursor-pointer"}/>
            <span>Add New</span>
          </div>
        </div>
